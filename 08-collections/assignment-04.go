...
func main() {
	...
	maxSize, maxCount := getMaxCountAndSize(wordsCountBySize)
...
func getWordsCountBySize(words []string) map[int]int {
	...
	for _, word := range words {
		countBySize[len(word)]++
	}
	return countBySize
}

func getMaxCountAndSize(countBySize map[int]int) (maxCount, maxSize int) {
	for size, count := range countBySize {
		if count > maxCount {
